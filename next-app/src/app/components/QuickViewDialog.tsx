...
import { durationsMs, easings, msToSeconds } from "../lib/motion";
...
export function QuickViewDialog({ product, open, onClose }: QuickViewDialogProps) {
  ...
  useEffect(() => {
    ...
    const locked = open || isPresenting;
    if (!locked) {
  ...
  }, [open, isPresenting, onClose, trapTab]);

  if (typeof document === "undefined") return null;
  if (!open && !isPresenting) return null;
  ...
  const backdropTransition = reduceMotion
    ? { duration: 0 }
    : {
        duration: msToSeconds(durationsMs.quickViewBackdrop),
        ease: easings.soft,
      };
  const panelTransition = reduceMotion
    ? { duration: 0 }
    : {
        duration: msToSeconds(durationsMs.quickViewPanel),
        ease: easings.out,
      };
