...
export function CheckoutClient({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      try {
        data = responseText
          ? (JSON.parse(responseText) as CheckoutApiBody)
          : {};
      } catch {
        console.error(
          "[checkout] create-checkout-session non-JSON body",
          responseText,
        );
        data = { detail: responseText || undefined };
      }

      if (!response.ok || !data.sessionId) {
        console.error(
          "[checkout] create-checkout-session failed (full JSON)",
          JSON.stringify(
            {
      ...
      console.error("[checkout] request or redirect failed", err);
      setError(
