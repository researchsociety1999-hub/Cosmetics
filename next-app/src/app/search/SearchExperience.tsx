        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {products.map((product) => {
            ...
            return (
              <article
                key={product.id}
                className="group relative flex h-full flex-col overflow-hidden rounded-[24px] border border-[rgba(214,168,95,0.18)] bg-[#0b0e14] shadow-[0_18px_45px_rgba(0,0,0,0.45)] transition duration-500 hover:-translate-y-1 hover:border-[rgba(214,168,95,0.45)]"
              >
                <Link
                  href={`/products/${product.slug}`}
                  className="relative block aspect-[4/5] overflow-hidden bg-[#11151d]"
                >
                  <Image
                    src={imageSrc}
