...
interface GetProductsOptions {
  ...
  /**
   * When true (default), products where every variant has stock = 0 or null are hidden.
   * Pass `false` to bypass — e.g. admin views that should show all products.
   */
 ...
 * Products with NO variants at all are treated as in-stock (stock field lives
 * on the variant, so absence of variants means the product itself has no
 * stock tracking — show it).
 */
function productIsInStock(product: Product): boolean {
  const variants = product.variant_stocks;
  // No variants joined → no stock tracking on this product → show it.
...
function filterMockProducts(
  ...
  const { categoryId, search, ingredientId, excludeComingSoon, excludeOutOfStock = true } = options;
  ...
  if (excludeOutOfStock) {
    filtered = filtered.filter(productIsInStock);
  }
