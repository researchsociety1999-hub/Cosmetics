...
export async function ShopCatalogBody({
  ...
  // Track a global card index across all sections so we only prioritise
  // the first 4 product images (the ones most likely to be above the fold).
  ...
  return (
    <div className="space-y-[4.5rem] md:space-y-[5.5rem]">
      ...
      {productSections.map((section) => (
        <section
          key={section.key}
          className="space-y-7 border-t border-white/[0.035] pt-11 first:border-t-0 first:pt-0 md:space-y-8 md:pt-14 md:first:pt-0"
        >
          <div className="space-y-3">
            <p className="text-[0.66rem] text-[#7d7368]">
              <span className="tabular-nums text-[#a69884]">
            ...
            <h2 className="font-literata text-[1.65rem] font-light leading-snug tracking-[0.1em] text-[#f0ebe4] md:text-3xl md:tracking-[0.09em]">
              {section.title}
            </h2>
          </div>
          <div
          ...
          >
            {[...section.products]
              ...
              .map((product) => {
                // Preload only the first 4 cards — covers the above-fold viewport
                // on both mobile (1-col) and desktop (3-4 col) layouts.
