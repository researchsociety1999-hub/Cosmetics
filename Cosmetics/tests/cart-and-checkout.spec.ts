...
test.describe("cart and checkout", () => {
  ...
  test("guest can update cart quantity and totals from the bag", async ({ page }) => {
    ...
    const lineArticle = page.getByRole("article").filter({ hasText: "Celestial Glow Serum" });
    const quantityInput = lineArticle.locator('input[id^="qty-"]');
    await expect(quantityInput).toBeVisible();
    await quantityInput.click({ clickCount: 3 });
    await quantityInput.pressSequentially("2", { delay: 30 });
    await expect(quantityInput).toHaveValue("2");

    const updateButton = lineArticle.getByRole("button", { name: "Update" });
    await expect(updateButton).toBeVisible();
    await updateButton.click();
    await page.waitForLoadState("load");
    // Mock Celestial Glow Serum: sale $58 × qty 2 → line total $116.00; bag summary matches.
    await expect(lineArticle.getByText(/^Line total:/)).toContainText("$116.00", { timeout: 60_000 });
    await expect(quantityInput).toHaveValue("2");
    const summaryAside = page.locator("aside").filter({ has: page.getByRole("heading", { name: "Summary" }) });
    await expect(summaryAside).toContainText("Items");
    await expect(summaryAside).toContainText("2");
  });
